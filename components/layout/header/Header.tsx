...
export default function Header({ navigation }: HeaderProps) {
  ...
  return (
    <header className="fixed inset-x-0 top-0 z-[999]">
      {/* Main bar */}
      <div className={["bg-[#0f1c3d] transition-shadow duration-300", scrolled ? "shadow-[0_4px_28px_rgba(10,18,40,0.32)]" : ""].join(" ")}>
        <div className="mx-auto flex h-[72px] max-w-[1280px] items-center justify-between gap-5 px-8 lg:px-12">
          
          {/* Logo */}
          <Link href="/" aria-label="Unit AA — Administratie & Advies" className="flex-shrink-0 transition-opacity duration-200 hover:opacity-90">
          ...
          <div className="hidden items-center gap-4 lg:flex">
            <Link href="/faq" className="rounded-lg px-[11px] py-[6px] text-[15px] font-medium text-white/55 transition-colors hover:text-white/85">
              FAQ
            </Link>
            <LanguageSwitcher />
            <Link
              href="/contact"
              className="group inline-flex items-center gap-[7px] rounded-[10px] bg-[#3db54a] px-[17px] py-[9px] text-[15px] font-bold text-white transition-all duration-150 hover:-translate-y-px hover:bg-[#30a83d] hover:shadow-[0_6px_18px_rgba(61,181,74,0.32)] active:translate-y-0 active:shadow-none"
            >
              Plan gratis gesprek
              <span className="flex h-[18px] w-[18px] items-center justify-center rounded-[5px] bg-white/18 text-[11px] transition-transform group-hover:translate-x-0.5">→</span>
            </Link>
          </div>
...
>
  ...
  <span className={["block h-[1.5px] w-[17px] origin-center rounded-full bg-white/70 transition-transform duration-200", mobileMenuOpen ? "-translate-y-[6.5px] -rotate-45" : ""].join(" ")} />
      ...
      {/* Mobile menu */}
      {mobileMenuOpen && (
        <div className="relative z-[999] border-t border-white/6 bg-[#0b1630] lg:hidden">
